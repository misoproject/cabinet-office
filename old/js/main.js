...
function setupGrouping(columns) {
  ...
  for(var i = 0; i < columns.length; i++){
    options += '<option value="' + columns[i].name + '">' + 
               columns[i].name + 
               '</option>';
  }
  
  $("#groupby").html(options)
    .change(function(){
...
function setupDateRanges(data){
  ...
  for(var i = 0; i < dateRanges.length; i++){
    options += '<option value="' + dateRanges[i] + '">' + 
                dateRanges[i] + 
                '</option>';
  }
...
function showExpenses(range, grouping, maxGroups){
  ...
  var rangeSelector = (range == wholeRange) ? 
    selectWholeRange : 
    function(row){ 
